...
class Unit:
    ...
    def check_for_task_completion(self, game_map, player):
        ...
        for ind, (action, target, *extra) in enumerate(self.task_q):
            # if player.current_strategy == StrategyTypes.STARTER and (action == ValidActions.BUILD) and (game_map.position_to_cluster(target) is None):
            if self.current_strategy == StrategyTypes.STARTER and (action == ValidActions.BUILD) and (game_map.position_to_cluster(target) is None or (not STRATEGY_HYPERPARAMETERS["STARTER"]["CONTINUE_TO_BUILD_AFTER_RESOURCES_DEPLETED"] and game_map.position_to_cluster(target).total_amount <= 0)):
